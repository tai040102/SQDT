...
from models.transformer import build_transformer,build_hoi_transformer
...
class DETRHOI(nn.Module):
    ...
    def __init__(self, backbone, transformer, hoi_transformer, num_obj_classes, num_verb_classes, num_queries, aux_loss=False):
        super().__init__()
        self.num_queries = num_queries
        self.transformer = transformer
        self.hoi_transformer = hoi_transformer
        hidden_dim = transformer.d_model
        self.query_embed = nn.Embedding(100, hidden_dim)
        
        self.h_query_embed = nn.Embedding(100, hidden_dim)
        self.o_query_embed = nn.Embedding(100, hidden_dim)
        ...
        self.pos_guided_embedd = nn.Embedding(100, hidden_dim)
        self.attn = nn.MultiheadAttention(hidden_dim, 8, dropout=0.1)
        self.ins_weights = nn.Parameter(data=torch.Tensor(3), requires_grad=True)
    ...
    def forward(self, samples: NestedTensor):
        ...
        h_query_embed = self.h_query_embed.weight + self.pos_guided_embedd.weight
        o_query_embed = self.o_query_embed.weight + self.pos_guided_embedd.weight
        ins_query_embed = torch.cat((h_query_embed, o_query_embed), dim=0)
        hs, memory = self.transformer(self.input_proj(src), mask, ins_query_embed, pos[-1])
...
#        ins_guided_embed = ins_guided_embed.permute(0, 2, 1, 3)
        h_hs = hs[:, :, :100, :]
        o_hs = hs[:, :, 100:, :]
        h_hs_temp = h_hs[-1].permute(1,0,2)
        o_hs_temp = o_hs[-1].permute(1,0,2)
        attn_score = self.attn(h_hs_temp, o_hs_temp, value=o_hs_temp)[0]
        ins_weights = self.soft_max(self.ins_weights)
        ins_guided_embed = ins_weights[0]*h_hs_temp + ins_weights[1]*o_hs_temp + ins_weights[2]*attn_score
        inter_hs = self.hoi_transformer(self.input_proj(src), mask, ins_guided_embed, pos[-1], memory, check_hoi=True)

        
        outputs_obj_class = self.obj_class_embed(o_hs)
        outputs_verb_class = self.verb_class_embed(inter_hs)
        outputs_sub_coord = self.sub_bbox_embed(h_hs).sigmoid()
...
def main(args):
    ...
    hoi_transformer=build_hoi_transformer(args)
    model = DETRHOI(backbone, transformer,hoi_transformer, len(valid_obj_ids) + 1, len(verb_classes),
                    args.num_queries)
